...
class Cipher {
 public:
  ...
  void Encrypt(void* buffer, size_t length) {
    ...
    encoder_->ProcessData((unsigned char*)buffer, (const unsigned char*)buffer, length);
  }
  ...
  void Decrypt(void* buffer, size_t length) {
    ...
    decoder_->ProcessData((unsigned char*)buffer, (const unsigned char*)buffer, length);
  }
